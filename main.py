...
@app.post("/predict")
async def predict(
    user_id: int, tag: int, file: UploadFile = File(...), db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if browse_id_exist > 0:
            ...
            return schemas.CustomResponse(
                code=200, message="浏览记录更新成功", data=data
            )
    # 浏览记录不存在
    except Exception:
        # 创建浏览记录
        try:
            data = schemas.BrowseBase(
                browse_id=crud.create_browse(db, user_id, bird_id)
            )
            return schemas.CustomResponse(
                code=200, message="浏览记录创建成功", data=data
            )
        # 创建失败
        except Exception as e:
