...
def search_bird(db: Session, bird_info: str):
    ...
    for attr in attributes:
        orm_result = (
            ...
            .filter(getattr(models.Bird, attr) == bird_info)
            .first()
        )
        if orm_result:
            return orm_result
...
def create_user(db: Session, username: str, password: str):
    add_user = models.User(username=username, password=password)
    db.add(add_user)
    db.commit()
    added_user = db.query(models.User).filter(models.User.username == username).first()
...
# 查询用户名是否存在
def username_has_exist(db: Session, username: str):
